// ─────────────────────────────────────────────────────────────
// OpenBreath — lifted from NextUs, redecorated to the white skin.
// Logic unchanged: descending three-centre open breath carried by the
// pacer circle. N rounds, each through three centres, each centre four
// phases (in / hold / exhale on "ah" / hold). Audio cues, pause / restart.
// Only palette and type changed.
//   onComplete() required · rounds (3) · centres · phases · sound
// ─────────────────────────────────────────────────────────────
...
const T = {
  ...
  goldFaint: 'rgba(19,156,221,0.20)', goldFill: 'rgba(19,156,221,0.10)',
...
export default function OpenBreath({
  ...
  function doPause() {
    setPaused(p => { const next = !p; pausedRef.current = next; return next })
  }
  function restart() {
    cancelAnimationFrame(rafRef.current)
      ...
      <div style={{ minHeight: '360px', marginTop: '14px' }}>
        {screen === 'running' && (
          <div>
            <div style={{ position: 'relative', width: '176px', height: '176px', margin: '0 auto 22px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              <div ref={phaseRef} style={{ ...BODY, fontWeight: 500, color: T.dark, fontSize: '19px', letterSpacing: '0.01em', lineHeight: 1.25, position: 'relative', zIndex: 1 }} />
            </div>

            <div ref={roundsRef} style={{ display: 'flex', justifyContent: 'center', gap: '8px', marginBottom: '6px' }} />
            <div ref={labelRef} style={{ ...SC, fontSize: '12px', fontWeight: 600, letterSpacing: '0.16em', color: T.whisper, textTransform: 'uppercase' }} />

            <div style={{ marginTop: '26px' }}>
              <div style={{ ...BODY, fontSize: '15px', fontWeight: 400, color: T.ghost, marginBottom: '4px' }}>Breathe into your…</div>
              <div ref={bodyRef} style={{ ...SERIF, fontWeight: 700, fontSize: 'clamp(30px,6vw,40px)', color: T.dark, lineHeight: 1.1, transition: 'opacity 0.6s ease' }} />
            </div>
          </div>
        )}
        ...
        {screen === 'done' && (
          <div style={{ padding: '60px 0' }}>
            <span style={{ ...SC, fontSize: '12px', fontWeight: 700, letterSpacing: '0.16em', color: T.gold, textTransform: 'uppercase', display: 'block', marginBottom: '12px' }}>{doneTitle}</span>
            <h1 style={{ ...SERIF, fontWeight: 700, fontSize: 'clamp(20px,2.4vw,26px)', color: T.gold, lineHeight: 1.15, margin: 0 }}>{doneHeading}</h1>
          </div>
        )}
      </div>
