// ─────────────────────────────────────────────────────────────
// ChargeBreath — lifted from NextUs, redecorated to the white skin.
// Logic is unchanged: Tabata-style charge breath, N rounds of fast
// breathing (work) separated by rest, a 3·2·1 countdown into round one,
// audio cues, a ring round-clock, the core pulsing as the exhale pacer,
// pause / restart. Only the palette and type were changed.
//
//   onComplete()   required. Called when the user finishes (Done).
//   onBack()       accepted; not surfaced.
//   rounds (3) · workSeconds (20) · restSeconds (10) · countdownSeconds (3) · sound
// ─────────────────────────────────────────────────────────────
import { useState, useEffect, useRef } from 'react'

const T = {
  dark: '#1D1D1F',
  gold: '#139CDD', goldChrome: '#0FA8C9',
  goldFaint: 'rgba(19,156,221,0.20)', goldTint: 'rgba(19,156,221,0.05)',
  goldStrong: 'rgba(19,156,221,0.30)',
  ghost: 'rgba(29,29,31,0.62)', whisper: 'rgba(29,29,31,0.40)',
}
const SC = { fontFamily: "'Montserrat', system-ui, sans-serif" }
const BODY = SC
const SERIF = SC
...
export default function ChargeBreath({
  ...
  function startReady(r) {
    ...
    if (r > 1) { beepBegin(); startWork(r); return }
    beepReady()
  ...
  return (
    <div style={{ maxWidth: '520px', margin: '0 auto', textAlign: 'center' }}>
      ...
      <span style={{ ...SC, fontSize: '12px', fontWeight: 700, letterSpacing: '0.16em', color: T.gold, textTransform: 'uppercase', display: 'block', marginBottom: '6px' }}>{title}</span>
      <h1 style={{ ...SERIF, fontWeight: 700, fontSize: 'clamp(24px,3vw,30px)', color: T.dark, lineHeight: 1.15, letterSpacing: '-0.01em', margin: 0 }}>{heading}</h1>

      <div style={{ minHeight: '300px', marginTop: '14px' }}>
        {phase === 'ready' && (
          <div style={{ padding: '36px 0' }}>
            <span style={{ ...SC, fontSize: '12px', fontWeight: 700, letterSpacing: '0.16em', color: T.gold, textTransform: 'uppercase', display: 'block', marginBottom: '16px' }}>Round {round} of {rounds} · {title}</span>
            <div style={{ ...BODY, fontSize: 'clamp(60px,14vw,88px)', fontWeight: 700, color: T.gold, lineHeight: 1, marginBottom: '16px' }}>{tick}</div>
            <p style={{ ...BODY, fontSize: '15.5px', fontWeight: 400, color: T.ghost, margin: 0 }}>Ready…</p>
          </div>
        )}

        {isActive && (
          <div style={{ padding: '8px 0' }}>
            <span style={{ ...SC, fontSize: '12px', fontWeight: 700, letterSpacing: '0.16em', color: T.gold, textTransform: 'uppercase', display: 'block', marginBottom: '20px' }}>
            ...
            <div style={{ position: 'relative', width: '210px', height: '210px', margin: '0 auto 20px' }}>
              ...
              <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', pointerEvents: 'none' }}>
                <span style={{ ...BODY, fontSize: '30px', fontWeight: 600, color: circleColor, lineHeight: 1 }}>{tick}</span>
              </div>
            </div>
            <p style={{ ...BODY, fontSize: '15.5px', fontWeight: 400, color: phase === 'rest' ? T.ghost : 'rgba(29,29,31,0.88)', margin: 0 }}>
        ...
        {phase === 'done' && (
          <div style={{ padding: '40px 0' }}>
            <span style={{ ...SC, fontSize: '12px', fontWeight: 700, letterSpacing: '0.16em', color: T.gold, textTransform: 'uppercase', display: 'block', marginBottom: '14px' }}>Charged</span>
            <h1 style={{ ...SERIF, fontWeight: 700, fontSize: 'clamp(20px,2.4vw,26px)', color: T.gold, lineHeight: 1.15, margin: 0 }}>System is awake.</h1>
          </div>
        )}
      </div>
